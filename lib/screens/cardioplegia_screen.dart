  ...
  Widget _alarmSettings() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      if (!_notificationsAvailable) ...[
        ...
        // No "retry" on platforms where it can never succeed
        // (Windows/Linux, audit R-2). A button that is guaranteed to fail
        // sends troubleshooting in the wrong direction — the message next to
        // it already says the platform is unsupported.
  ...
  return [
    PdfSection(title: t('pdf_inputs'), rows: [
      ...
      if (delNido) ...[
        ...
        // verify:ok dnPct comes from a setting, but is covered here by the
        // enclosing `if (delNido)` — the row only exists when del Nido input
        // is actually present. The tab therefore cannot appear permanently
        // "filled" (cf. C-1 in the pediatric tab).
    ...
    PdfSection(title: t('pdf_results'), rows: [
      if (calafiore) ...[
        ...
        // resultIf + zeroIsValid, as with the electrolytes: on screen these
        // cards show a 0 once the inputs are complete, while the PDF showed
        // "—".
        //
        // The important case is the perfusor rate: calafioreDeltaK is
        // clamped at 0 when the serum potassium has already reached the
        // target. The screen then reads "0.0 ml/h" AND displays the
        // cardio_no_dose_needed hint. The PDF showed "—" and the hint not at
        // all — a reader could not tell whether the calculation had failed
        // or no supplementation was needed. The hint therefore moves into
        // the row as a note.
        //
        // Magnesium is optional: 0 means "no Mg in the syringe" or "no Mg
        // delivery" here, not "not calculable". Availability therefore
        // depends on the KCl inputs only.
